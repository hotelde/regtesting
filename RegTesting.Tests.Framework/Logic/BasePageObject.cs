...
namespace RegTesting.Tests.Framework.Logic
{
	abstract public class BasePageObject
	{
		...
		internal void WaitFor(BasicPageElement element, Visibility visibility = Visibility.Visible)
		{
		...
		/// <returns>T</returns>
		internal T GetDynamicElement<T>(By findBy, WaitModel waitModel = null, ClickBehaviours clickBehaviour = ClickBehaviours.Default) where T : BasicPageElement
		{
		...
		internal void ChangeFrame(string frameName)
		{
		...
		internal void SwitchToTab(string urlSequence)
		{
